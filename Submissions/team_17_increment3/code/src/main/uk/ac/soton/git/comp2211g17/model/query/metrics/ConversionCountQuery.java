...
public class ConversionCountQuery extends KeyMetricQueryBase {
	public static final IntegerField count = new IntegerField("Number of Conversions");

	private final DatabaseManager dbm;

	public ConversionCountQuery(DatabaseManager dbm) {
		super(count);
		this.dbm = dbm;
	}
		...
		if (getInterval() > 0) {
			...
			Result<Record2<Integer, Integer>> result = create.select(time, field("count").cast(SQLDataType.INTEGER)).from(
				create.select(divTime, count).from(applyUserJoin(SRV))
					.where(SRV.CONVERSION.isTrue().and(getFilterCondition(SRV))).groupBy(divTime)
			).fetch();
			...
			Result<Record1<Integer>> result = create.select(
				count(asterisk())
			).from(applyUserJoin(SRV)).where(SRV.CONVERSION.isTrue().and(getFilterCondition(SRV))).fetch();
	...
	public QueryPlan buildQueryPlan() {
		return new QueryPlan(copyTo(new ConversionCountQuery(dbm))::execute);
	}
}
