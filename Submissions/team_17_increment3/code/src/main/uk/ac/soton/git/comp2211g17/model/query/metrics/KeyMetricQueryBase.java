...
public abstract class KeyMetricQueryBase implements KeyMetricQuery {
	...
	protected Condition getFilterCondition(Table<?> table) {
		if (filter != null) {
			Filter filterValue = filter.apply(table);
			if (filterValue != null) {
				return filterValue.getCondition();
			}
		}
	...
	// TODO: should the fields be preserved?
	protected <T extends KeyMetricQueryBase> T copyTo(T other) {
		other.setFilter(filter);
