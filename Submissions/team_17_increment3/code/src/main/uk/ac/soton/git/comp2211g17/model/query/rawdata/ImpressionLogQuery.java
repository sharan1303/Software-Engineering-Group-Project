...
public class ImpressionLogQuery implements Query, PaginatableQuery {
	...
	public static final FloatField impressionCost = new FloatField("Impression Cost");
	...
	public Field<?>[] getFields() {
		return new Field<?>[]{date, id, gender, audienceAge, incomeGroup, context, impressionCost};
	}

	public Column<?>[] execute() {
		DSLContext create = DSL.using(dbm.conn, SQLDialect.SQLITE);
		Result<Record> result = create.select().from(IMPR).join(USER)
			.using(USER.ID).limit(limit).offset(offset).fetch();

		return new Column<?>[]{
			date.fromJooqField(result, IMPR.DATE),
			id.fromJooqField(result, IMPR.ID),
			...
			impressionCost.fromJooqField(result, IMPR.IMPRESSIONCOST)
		};
	}
	...
	public QueryPlan buildQueryPlan() {
		ImpressionLogQuery query = new ImpressionLogQuery(dbm);
		query.limit = limit;
		query.offset = offset;
		return new QueryPlan(query::execute);
	}
}
